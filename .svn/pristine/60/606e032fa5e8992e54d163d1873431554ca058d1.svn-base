	...
	var callback = function(res){
		...
		$("#jsGrid").jsGrid({
			...
			fields: [
				...
				{ name: "emp_no", title: "사번", type: "text", width: 30, align: "center" },
			...
			rowClick: function(args) {
				...
				$("#view_debit_code").val(item.debit_code);
				$("#view_crebit_code").val(item.crebit_code);
...
function expenseOutput(){
	...
	opener.onload = function() {
		...
		$(opener.document).find('#exp_pay').text($('#view_exp_pay').val());
...
function expenseUpdate(){
	var exp_pay = $("#view_exp_pay").val();
	var param = {			
			checkApproval: $('input:radio[name="checkApproval"]:checked').val(),
			exp_id: $("#view_exp_id").val(),
			emp_name: $("#view_emp_name").val(),
			use_date: $("#view_use_date").val(),
			client_name: $("#view_clientId").val(),
			voucher_amount: parseInt(exp_pay.replace(/,/g, '')),
			crebit_code: $("#view_crebit_code").val(),
			debit_code: $("#view_debit_code").val(),
	}
	if(!param.checkApproval || param.checkApproval ===""){
		alert("승인여부를 선택해주세요.");
		return;
	}
	var callback = function(res){
	...
	callAjax("/account/expenseLastUpdate.do", "post", "json", false, param, callback);
}
			...
			<ul>
				...
				<li class="contents">
					...
					<p class="conTitle" style="height: 110px">
						<span style="text-align: left; display: block;">지출결의서 승인/조회</span> 
						<span >					
 	                         	 신청일자
...
<!-- 조회모달팝업 -->
	...
	<input type="hidden" id="view_crebit_code"/> 
	<input type="hidden" id="view_debit_code"/> 	
		<dl>
			<dt>
				<strong>지출결의서</strong>
			</dt>
			<dd class="content">
				<table class="row">
					...
					<tbody>
						...
						<tr>
							...
							<input type="radio" 
								name="checkApproval" value="S"  checked="checked"/> 승인
				...
				<div class="btn_areaC mt30">
					...
					<a href="" class="btnType blue" id="btnUpdateExpense" name="btn"><span>승인</span></a> 
