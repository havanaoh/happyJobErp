...
<script type="text/javascript">
	...
	function getToday() {
		var today = new Date();
		$("#searchStDate").val(formatDate(today));
		$("#searchEdDate").val(formatDate(today));
	}
	
	function getMonthPeriod() {
		var stArr = $("#searchStDate").val().split("-");
		var edArr = $("#searchEdDate").val().split("-");
		
		var startYear = stArr[0];
		var startMonth = stArr[1];
		var endYear = edArr[0];
		var endMonth = edArr[1];
	...
	function monthlySearch(){
		if (getMonthPeriod() < 0) {
			alert("시작일이 끝나는 날 이후일 수 없습니다.");
			return;
		} else if (getMonthPeriod() > 5) {
			alert("6개월 이내의 데이터만 검색 가능합니다.");
			return;
		}
		...
		var callback = function(res){
			...
			$("#mainGrid").jsGrid({
				...
				data: detail,
				fields: [
					{ name: "orderDate", title: "날짜", type: "text", width: 80, align: "center" },
					{ name: "orderCount", title: "주문 건수", type: "number", width: 60, align: "center", itemTemplate: function(value) { return gfFormatNumberWithComma(value); } },
					{ 
		...
		callAjax("/sales/monthlyList.do", "post", "json", false, param, callback);
	}
	
	function getDetail(type) {
		var url = (type == 'product') ? "/sales/monthlyTopProduct.do" : "/sales/monthlyTopClient.do";
		var titleName = (type == 'product') ? "제품명" : "기업명";
		
		gfModalPop("#monthlyModal");
	...
	function mainChart(data) {
		...
		$.each(data.monthlyList, function(index, item) {
			...
			<li class="contents">
				...
				<div class="content">
					<p class="Location">
						...
						<span class="btn_nav bold">월별 매출현황</span> 
						...
						<table class="searchTable">
							...
							<tbody>
								<tr>
									<td>기간</td>
									<td>
										<input type="month" id="searchStDate" name="searchStDate" /> 
										<input type="month" id="searchEdDate" name="searchEdDate" />
									</td>
									<td style="text-align:right;">
										<a href="javascript:monthlySearch();" class="btnType gray" name="btn" id="searchBtn"><span>조회</span></a>
									</td>
								</tr>
							</tbody>
						</table>
...
<div id="monthlyModal" class="layerPop layerType2" style="width:900px;">
	<dl>
		<dt>
